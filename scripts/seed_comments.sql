-- ============================================================
-- Replace generic seed comments with topic-relevant ones.
-- Scope: the 10 seeded communities only. Leaves c/saas untouched.
-- ============================================================
...
SELECT
  (SELECT COUNT(*) FROM posts    WHERE community_id IN (SELECT id FROM communities WHERE name IN
    ('askreddit','programming','gaming','news','science','movies','books','food','travel','fitness'))) AS seeded_posts,
  (SELECT COUNT(*) FROM comments WHERE post_id IN
    (SELECT id FROM posts WHERE community_id IN (SELECT id FROM communities WHERE name IN
      ('askreddit','programming','gaming','news','science','movies','books','food','travel','fitness')))) AS comments_now,
  (SELECT COUNT(DISTINCT author_id) FROM comments WHERE post_id IN
    (SELECT id FROM posts WHERE community_id IN (SELECT id FROM communities WHERE name IN
      ('askreddit','programming','gaming','news','science','movies','books','food','travel','fitness')))) AS unique_authors;
